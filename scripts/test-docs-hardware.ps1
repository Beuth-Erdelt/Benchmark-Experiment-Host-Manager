...
Write-Host "$(Get-Date -Format 'yyyy-MM-dd HH:mm:ss') [DONE] 17. Hardware sockperf pod scaling sweep  mode=ul  protocol=tcp  msgsize=64  mps=max  nbp=1,2,4,8,16"
...
Write-Host "$(Get-Date -Format 'yyyy-MM-dd HH:mm:ss') [DONE] 18. Hardware sockperf PostgreSQL query round-trip latency  mode=pp  protocol=tcp  msgsize=64"
...
Write-Host "$(Get-Date -Format 'yyyy-MM-dd HH:mm:ss') [DONE] 19. Hardware sockperf PostgreSQL streaming throughput  mode=ul  protocol=tcp  msgsize=8192"
...
Write-Host "$(Get-Date -Format 'yyyy-MM-dd HH:mm:ss') [DONE] 20. Hardware sockperf PostgreSQL query latency under concurrency  mode=pp  protocol=tcp  msgsize=64  nbp=1,2,4,8,16"
