...
if [[ "$BEXHOMA_DBMS" == "redis" ]]; then
    ...
    then
        # report status
        time bin/ycsb load redis -P $FILENAME -P db.properties -cp jars/$BEXHOMA_JAR -p measurementtype=hdrhistogram -p -s
    else
        time bin/ycsb load redis -P $FILENAME -P db.properties -cp jars/$BEXHOMA_JAR -p measurementtype=hdrhistogram -p
    fi
