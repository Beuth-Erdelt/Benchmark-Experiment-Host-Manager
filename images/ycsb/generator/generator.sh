...
if [[ "$BEXHOMA_DBMS" == "redis" ]]; then
    ...
    then
        # report status
        time bin/ycsb load redis -P $FILENAME -P db.properties -cp jars/$BEXHOMA_JAR -p redis.cluster=true -s
    else
        time bin/ycsb load redis -P $FILENAME -P db.properties -cp jars/$BEXHOMA_JAR -p redis.cluster=true
    fi
else
    if test $YCSB_STATUS -ne 0
    then
