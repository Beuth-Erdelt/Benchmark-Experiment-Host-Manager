# TPC-H Refresh Loader — MariaDB

Applies TPC-H RF1 (INSERT) and RF2 (DELETE) operations to a MariaDB database.
...
| `BEXHOMA_HOST` | `www.example.com` | MariaDB host |
| `BEXHOMA_PORT` | `3306` | MariaDB port |
| `BEXHOMA_USER` | `root` | MariaDB user |
| `BEXHOMA_PASSWORD` | `root` | MariaDB password |
| `BEXHOMA_VOLUME` | `tpch` | Database name (overrides `BEXHOMA_DATABASE` at runtime) |
| `BEXHOMA_CONNECTION` | `mariadb` | Connection name (Redis counter key) |
