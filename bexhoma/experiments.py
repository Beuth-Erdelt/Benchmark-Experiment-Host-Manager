...
class default():
    ...
    def end_benchmarking(self, jobname, config=None):
        ...
        if len(job_labels) > 0 and len(job_labels[jobname]) > 0:
            ...
            print("{:30s}: showing benchmarker times".format(config.connection))
            print("{:30s}: benchmarker timespan (first to last [s]) = {}".format(config.connection, end_time-start_time))
            print("{:30s}: benchmarker times (start/end per pod and container) = {}".format(config.connection, timing_benchmarker))
