...
class YcsbCollector(CollectorBase):
    ...
    def get_evaluator(self, code=''):
        ...
        if code == '':
            code = self.codes[0]
        return evaluators.ycsb(code=code, path=self.path, benchmark_run=self.benchmark_run)

    def get_benchmark_timeseries_per_phase(self, metric="current_ops_per_sec"):
        """
        ...
        combination, calls :meth:`evaluators.ycsb.get_benchmark_logs_timeseries_df_aggregated`
    ...
    def get_loading_timeseries_per_phase(self, metric="current_ops_per_sec"):
        """
        Combines aggregated YCSB loading time-series per phase from all experiment codes into a wide-format DataFrame.

        For each code and each unique ``(configuration, experiment_run)`` combination,
        calls :meth:`evaluators.ycsb.get_loading_logs_timeseries_df_aggregated` and
        ...
        for code in self.codes:
            evaluation = self.get_evaluator(code)
            df_loading = evaluation.get_df_loading()
            if df_loading.empty:
                continue
            df_loading = evaluation.loading_set_datatypes(df_loading)
            for (configuration, experiment_run), _ in df_loading.groupby(
                ['configuration', 'experiment_run']
            ):
                df_ts = evaluation.get_loading_logs_timeseries_df_aggregated(
                    metric=metric,
                    configuration=configuration,
                    experiment_run=experiment_run
                )
                if isinstance(df_ts, pd.DataFrame) and not df_ts.empty and metric in df_ts.columns:
                    col_label = f"{code}-{configuration}-{experiment_run}"
                    df_result[col_label] = df_ts[metric]
        return df_result

    def get_loading_timeseries_all(self, metric="current_ops_per_sec"):
        """
        Collects long-format YCSB loading time-series data for a given metric across all experiment codes.

        For each code and each unique ``(configuration, experiment_run)`` combination,
        calls :meth:`evaluators.ycsb.get_loading_logs_timeseries_df_aggregated`, reshapes
        ...
        The YCSB evaluator uses ``'sec'`` as the index name; this method normalises it
        to ``'second'`` for consistency with other time-series methods.

        :param metric: YCSB metric to retrieve (default ``'current_ops_per_sec'``).
        :type metric: str
        :return: Long-format DataFrame with columns ``second``, ``code``, ``configuration``,
                 ``experiment_run``, ``metric``, ``value``, plus connection metadata columns.
        :rtype: pandas.DataFrame
        """
        ...
        for code in self.codes:
            ...
            if not df_code.empty:
                join_keys = ['code', 'configuration', 'experiment_run']
                extra_cols = [
                    c for c in df_connections.columns
                    if c not in join_keys and c not in ['connection', 'phase', 'client']
                ]
    ...
    def get_benchmark_timeseries_all(self, metric="current_ops_per_sec"):
        """
        ...
        calls :meth:`evaluators.ycsb.get_benchmark_logs_timeseries_df_aggregated`, reshapes
        the result to long format, and annotates each row with its identifying fields.
        Connection metadata (e.g. ``type_tenants``, ``num_tenants``, ``vol_tenants``) is joined
        in from :meth:`get_connections` for each code.
