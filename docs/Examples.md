...
```
cluster = masterK8s.testdesign(clusterconfig='cluster.config', experiments_configfolder='experiments/tpch', yamlfolder='k8s/')
```

On **AWS**:

```
cluster = masterAWS.testdesign(clusterconfig='cluster.config', experiments_configfolder='experiments/tpch')
```
