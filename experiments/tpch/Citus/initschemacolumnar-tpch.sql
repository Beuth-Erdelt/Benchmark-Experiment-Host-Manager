...
-- Purpose: Create TPC-H tables for Citus using the columnar storage engine
--          (citus_columnar) for the two largest tables: orders and lineitem.
--          Columnar storage reduces I/O for analytical full-table scans at the
--          cost of slower point lookups. Dimension tables remain row-store.
--          Otherwise identical to initschema-tpch.sql.
...
-- USING COLUMNAR enables the citus_columnar access method for analytical scans.
CREATE TABLE orders (
