...
-- Purpose: Configure replication and create TPC-H tables with inline primary
--          keys and secondary indexes following CockroachDB's recommended
--          schema for TPC-H. Uses FLOAT for price and quantity columns
--          (CockroachDB preference over DECIMAL for analytical workloads).
--          Indexes matching the TPC-H query workload are embedded in the schema.
