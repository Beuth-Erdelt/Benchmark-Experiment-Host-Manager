...
-- Purpose: Create secondary indexes on foreign key columns for TPC-H queries.
--          Run after data loading. Note: CockroachDB automatically creates a
--          backing index when a FK is added (initconstraints-tpch.sql), so an
--          explicit index on supplier(s_nationkey) is not needed here.
