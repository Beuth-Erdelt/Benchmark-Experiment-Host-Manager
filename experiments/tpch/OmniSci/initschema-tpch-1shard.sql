...
-- Purpose: OmniSci TPC-H schema with BIGINT keys, DECIMAL(16,2) monetary values,
--          and shard_count = 1 (single-node equivalent sharding).
--          customer and orders are sharded on the customer key so that joins
--          between them are co-located. lineitem has no shard key in this variant.
--          See initschema-tpch-2shards.sql / -4shards / -8shards for distributed
--          variants, and initschema-tpch-template.sql for the Bexhoma template.
