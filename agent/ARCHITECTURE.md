...
| Bare-model baseline | `prompts.py::baseline_messages`, `agent.py::run_baseline` | One toolless model call answering the question with no catalog, contract, or handbook; same trajectory-and-`answer.md` output; run by the lifecycle wrapper as its own investigation and linked from the design trajectory; toggled by `--baseline`/`--no-baseline` (`AGENT_BASELINE`), off by default |
