...
clean_logs() {
    ...
    echo "Found log files: ${#logFiles[@]}"

    for file in "${logFiles[@]}"; do
        echo "Processing file: $file"

        filename=$(basename "$file" .log)
        outputFile="$LOG_DIR/${filename}_summary.md"

        startLine=$(grep -n "## Show Summary" "$file" | head -1 | cut -d: -f1)

        if [ -n "$startLine" ]; then
            echo "Match found at line: $startLine"
            tail -n +"$startLine" "$file" > "$outputFile"
            echo "Summary written to: $outputFile"
        else
            echo "No summary found in $file"
        fi
    done
}
