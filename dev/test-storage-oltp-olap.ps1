...
#
# Revision 2 added an ANOMALY DIAGNOSTICS block per storage class, following
...
# The diagnostic profiles decompose each anomaly along the axes most likely
# to explain it (fio ioengine, queue depth, read/write mix, fsync on/off)
# instead of accepting each as a single unexplained data point.
#
# Revision 3 restructures execution from "run every test for cephcsi, then
# run every test for shared" to "for every test, run cephcsi then shared"
# (see $tests / the driver loop at the bottom). Round 2 ran fully sequential
# by storage class, and its plain 8k queue-depth sweep reversed its own
# ranking when re-run later the same day - hours of cluster-load/environment
# drift between "all of cephcsi's block" and "all of shared's block" is
# indistinguishable from a real storage-class difference under that
# structure. Interleaving means any such drift lands on both classes'
# measurement of the *same* test at close to the *same* time instead of
# contaminating one class's entire run and not the other's. Each test is
# now a function taking $storageClass as a parameter; the driver loop below
# calls every test once per entry of $BEXHOMA_STORAGE_CLASSES before moving
# to the next test.
#
...
function Test-OltpRandomPageCost {
    <#
    .SYNOPSIS
...
function Test-OltpWalSyncFsync {
    <#
    .SYNOPSIS
    OLTP 3. WAL sync-write commit latency (fsync).
    Repeated 3x (-nc 3): this single-round number anchors every later
    commit-latency comparison, and the first run measured it once.
    #>
    param([string]$storageClass)
...
function Test-OltpWalSyncFdatasync {
    <#
    .SYNOPSIS
    OLTP 4. WAL sync-write commit latency (fdatasync).
    Repeated 3x (-nc 3), same reasoning as OLTP 3.
    #>
    param([string]$storageClass)
...
function Test-OltpWalGroupCommit {
    <#
    .SYNOPSIS
    OLTP 5. WAL group-commit scaling (concurrent committing backends).
    Repeated 3x (-nc 3): the first run showed a noisy, non-monotonic cephcsi
    curve (51->16.6->93->82->137->391 iops across nbt=1..32) that looks like
    the same concurrent-fsync instability as the contention-proxy collapse
    (OLTP 7-9), not just measurement noise - worth confirming with repetition
    before concluding shared scales better here.
    #>
    param([string]$storageClass)
...
function Test-OltpContentionProxy {
    <#
    .SYNOPSIS
    ...
    anomaly diagnostics (OLTP 7-9) decompose.
    #>
    param([string]$storageClass)
...
###########################################
# cephcsi collapsed on OLTP 6 (~66/29 iops, ~2.4-2.9s p95/p99) while shared
# stayed healthy (519/221 iops, sub-second latency) - the inverse of every
# other OLTP result. The three functions below each vary one axis of OLTP 6
# to find what specifically triggers it.


function Test-OltpContentionEngineSweep {
    <#
    .SYNOPSIS
    OLTP 7. Contention: fio ioengine sweep.
    Rules out a fio/libaio artifact before blaming the storage backend: if
    io_uring doesn't collapse the same way at the same parameters, the
    finding is "libaio handles this contention pattern badly on cephcsi",
    not "cephcsi is bad under contention".
    ...
    images/hardware/benchmarker/run_fio.sh, which now logs fio's exit code
    and stderr and validates its JSON before parsing, so a repeat of this
    mistake fails loudly instead of silently). A synchronous
    single-outstanding-request baseline already exists at its own natural
    depth in OLTP 3/4 (WAL fsync/fdatasync, -xfid 1); it does not belong in
    a depth=64 comparison.
    #>
    param([string]$storageClass)
    ...
    bexhoma hardware `
      ...
      -xtd 60                          <# seconds per fio round #> `
      -xfrw randrw                     <# mixed random read/write, one queue, one profile #> `
      -xfmx 70                         <# read percentage, matches OLTP 6 #> `
      -xfbs 8k                         <# fio block size, matches OLTP 6 #> `
      -xfid 64                         <# queue depth, matches OLTP 6 #> `
      -xfe libaio,io_uring             <# ioengines to sweep (comma-separated); sync excluded, see comment above #> `
      -xfsy 1                          <# fsync after every write, matches OLTP 6 #> `
      -nbp 1                           <# benchmarking pod count #> `
      -nbt 1                           <# threads per benchmarking pod (fio numjobs) #> `
      -ne 1                            <# parallel client counts to sweep (comma-separated) #> `
      -m                               <# collect SUT resource metrics #> `
...
function Test-OltpContentionMixSweep {
    <#
    .SYNOPSIS
    OLTP 9. Contention: read/write mix sweep.
    Finds how much write fraction is needed to trigger the collapse - sweeps
    from all-read (100, should match OLTP 1's randread) to all-write (0,
    should match OLTP 1's randwrite), crossed with fsync on/off.
    #>
    param([string]$storageClass)
...
function Test-OlapSeqreadBlocksizeSweep {
    <#
    .SYNOPSIS
    ...
    reads, so this now covers the sizes Postgres itself can actually
    request, not just an OS-readahead proxy. Also extended up to 64M to
    check whether shared's 16M write cliff (OLAP 4) has a read-side
    counterpart at larger sizes still - the first run found no cliff on the
    read side up to 16M.
    Repeated 3x (-nc 3): this test ran once per storage class in round 2 and
    its cephcsi-vs-shared lean was cited in the PostgreSQL config
    recommendations - repeat it before trusting that lean, the same way
    group-commit's repeated numbers overturned round 1's single-shot
    conclusion there.
    #>
    param([string]$storageClass)
...
function Test-OlapSeqreadDepthSweep {
    <#
    .SYNOPSIS
    OLAP 2. Sequential-read queue-depth sweep (readahead effectiveness).
    Extended to 64,128 (was 1..32): neither class had plateaued by depth 32
    in the first run.
    Repeated 3x (-nc 3), same reasoning as OLAP 1.
    #>
    param([string]$storageClass)
...
function Test-OlapParallelScan {
    <#
    .SYNOPSIS
    OLAP 3. Parallel sequential-scan proxy (concurrent scan workers).
    Repeated 3x (-nc 3): unchanged parameters from round 1, but its "cephcsi
    scales better past 2 workers" finding was cited in the PostgreSQL config
    recommendations (max_parallel_workers_per_gather) - same
    repeat-before-trust reasoning as OLAP 1-2.
    #>
    param([string]$storageClass)
...
function Test-OlapBulkWrite {
    <#
    .SYNOPSIS
    OLAP 4. Bulk sequential-write throughput (bulk load / checkpoint writeback).
    Extended -xfbs to localize shared's 16M write cliff (first run only
    sampled 1M, 4M, and 16M - the cliff is somewhere in the 4M-16M gap).
    #>
    param([string]$storageClass)
...
function Test-OlapContentionProxy {
    <#
    .SYNOPSIS
...
###########################################
# shared collapsed at bs=16M in OLAP 4 (~1.1-1.2 iops, 4-13s p99 latency)
# while cephcsi kept working normally (9-30 iops). The function below fixes
# bs=16M and sweeps queue depth (crossed with fsync on/off, to approximate
# archiving/copying a full 16MB WAL segment with an fsync at the end) to see
# whether the collapse needs concurrency or already happens at a single
# outstanding 16M write.


function Test-OlapBulkWriteCliffDepth {
    <#
    .SYNOPSIS
    ...
    bexhoma hardware `
      ...
      -xfbs 16M                        <# fio block size, fixed at the cliff found in OLAP 4 #> `
...
############### Driver loop ###############
###########################################
# Outer loop over tests, inner loop over storage classes: interleaves
# cephcsi/shared per test instead of running one class's entire block before
# starting the other's (see revision 3 note at the top of this file).

$tests = @(
    'Test-OltpDepthSweep',
    'Test-OltpRandomPageCost',
    'Test-OltpWalSyncFsync',
    'Test-OltpWalSyncFdatasync',
    'Test-OltpWalGroupCommit',
    'Test-OltpContentionProxy',
    'Test-OltpContentionEngineSweep',
    'Test-OltpContentionDepthSweep',
    'Test-OltpContentionMixSweep',
    'Test-OlapSeqreadBlocksizeSweep',
    'Test-OlapSeqreadDepthSweep',
    'Test-OlapParallelScan',
    'Test-OlapBulkWrite',
    'Test-OlapContentionProxy',
    'Test-OlapBulkWriteCliffDepth'
)

foreach ($testName in $tests) {
    foreach ($storageClass in $BEXHOMA_STORAGE_CLASSES) {
        Write-Host "$(Get-Date -Format 'yyyy-MM-dd HH:mm:ss') [START] $testName / $storageClass"
        & $testName $storageClass
    }
}
